...
from common.storage import PostgresTaskStore, TaskStore, record_to_dict
...
def build_orchestrator() -> AIOpsOrchestrator:
    ...
    if rag_mode == "json":
    ...
    elif rag_mode == "qdrant":
        rag_service = RAGService(
            QdrantVectorStore(
                os.getenv("AIOPS_VECTOR_DB_URL", ""),
                collection=os.getenv("AIOPS_VECTOR_DB_COLLECTION", "aiops_runbooks"),
            ),
            llm_client=llm_client,
        )
    elif rag_mode == "none":
        ...
        raise ValueError("AIOPS_RAG 只能是 none、json、http 或 qdrant")
    ...
    storage_mode = os.getenv("AIOPS_STORAGE", "sqlite").strip().lower()
    if storage_mode == "postgres":
        store = PostgresTaskStore(os.getenv("AIOPS_DATABASE_URL", ""))
    elif storage_mode == "sqlite":
        store = TaskStore(database_path=data_path / "aiops_tasks.sqlite3")
    else:
        raise ValueError("AIOPS_STORAGE 只能是 sqlite 或 postgres")
